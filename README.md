...
| Gadget | 依赖 | JDK 版本 | 描述 |
|--------|------|----------|------|
| `CommonsCollections1` | commons-collections:3.1 | JDK <= 8u71 | CommonsCollections1 |
| `CommonsCollections3` | commons-collections:3.1 | JDK <= 8u71 | CommonsCollections1的变化，使用 InstantiateTransformer 绕过 InvokerTransformer |
| `CommonsCollections6_3` | commons-collections:3.1 | - | CommonsCollections6前半段 + CommonsCollections3后半段，使用 InstantiateTransformer 绕过 InvokerTransformer |

### 字节码 Gadget

| Gadget | 描述 |
|--------|------|
| `字节码分离加载` | 适用于字节码长度限制, 通过环境变量获取内存马切片并加载 |
...
### v1.0.0 (2024-08-17)
- 初始版本
- 添加 CommonsCollections1/3/6_3 反序列化链
